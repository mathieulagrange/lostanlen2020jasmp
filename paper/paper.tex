...
Consequently, regardless of mute and technique, the number of instruments in our study is equal to $I=15$.
...
\subsection*{Inter-instrument and inter-technique similarities}
...
To answer this question, we derive from the consensus clustering graph $\mathcal{G}_0$ an instrument-wise similarity matrix $\mathbf{A}$, containing $I=15$ rows and $I$ columns.
For every instrumment--instrument pair $(i,j)$, we set the value of $\mathbf{A}}$ at row $i$ and column $j$ equal to the number of pairs in $\mathcal{G}_0$ from instruments $i$ and $j$ respectively, renormalized by the number of IMT samples from either instrument $i$ or instrument $j$:
\[
\mathbf{A}(i,j) = \dfrac{
