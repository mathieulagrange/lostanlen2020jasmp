...
This observation has led auditory neuroscientists to formulate an idealized computational model for STRF, known as ``full cortical model'' \cite{chi2005jasa}, which densely covers the scale-rate domain $(\alpha, \beta)$.
...
These biologically inspired features have shown to outperform the state of the art, especially in the small data regime when deep learning is inapplicable. % TODO cite?
...
In other words, although there is no evidence that the full cortical model consistently predicts the same instrument labels as a human listener, cross-instrument confusions do occur in the same proportions overall.
%Lastly, beyond the realm of supervised classification, % todo cite thoret, cite hemery
%For these reasons, STRFs offer an appealing starting point towards the development of a query-by-example search engine for instrumental playing techniques.
%Yet, the full cortical model 
