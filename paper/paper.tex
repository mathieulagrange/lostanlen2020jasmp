...
It encodes patterns of spectrotemporal modulation in the query while offering numerical guarantees of stability to local deformations \cite{anden2012scattering}.
...
The reason behind this choice is that most percussive instruments do not allow much variation in terms of pitch or intensity, if at all; thus reducing the amount of intra-technique variability in the time--frequency domain. \ml{preciser le contexte, un joueur de tabla ne serait pas d'accord, j'enleverai ces 2  phrases, ... and refer to ...}
...
The Studio On Line dataset (SOL), recorded at Ircam in 1998, offers a dense sampling of this \ml{repetition : combinatorial explosion, what would be the complete, give the percent achieved of SOL}, amounting to 25444 audio signals in total.
...
Conversely, the value 7 (most relevant) denotes that the IMT under consideration has a strong similarity with IMTs from one or several other instruments \ml{available in the database}.

Once both experts completed their annotation, we retained all IMTs whose \ml{relevance score averaged over experts} was judged equal to 3 or higher.
...
In May and June 2016, we recruited volunteers to use Cyberlioz on their own computers, via a web browser, and equipped with a pair of earphones. % TODO give the exact two months.
We publicized this study on \ml{internal student mailing list of the cnsmdp} international mailing lists for research in music audio processing, such as Auditory and Music-IR. % TODO give mailing lists
Within two months, $K=31$ subjects accessed Cyberlioz and completed the task. % TODO break down by gender? give age stats? musical experience? did not kept this info sorry
...
=\left\{
\begin{array}{ll}
\mathbf{S_{1}}\boldsymbol{x}(t,\lambda,\alpha,\beta) & \textrm{if }\alpha\neq0,\\
...
This vector includes both first-order and second-order scattering coefficients, after median-based logarithmic compression and affine standardization (see subsectio\ml{s} above).
...
\end{equation}
Intuitively, the term $\mathcal{E}_{\textrm{pull}}$ tends to shrink local Euclidean neighborhoods in feature space. Crucially, in the case of time--frequency scattering coefficients, these Euclidean neighborhoods corresponds to Wasserstein neighborhoods in the scalogram domain (see previous subsection).
...
Conversely, the term $\mathcal{E}_{\textrm{push}}$ tends to penalize small distances between samples that belong to different clusters in $\mathcal{G}$.
More precisely, consider three samples $\boldsymbol{x}$, $\boldsymbol{y}$ and $\boldsymbol{z}$ in $\mathcal{X}$.
Assume that the former belongs to the Euclidean neighborhood of $\boldsymbol{x}$ (i.e., $\boldsymbol{y} \in \mathcal{Y}_R (\boldsymbol{x})$) while the latter belongs to a different cluster as $\boldsymbol{x}$ (i.e., $\boldsymbol{z}\not\in\mathcal{G}(\boldsymbol{x}$)).
The term $\mathcal{E}_{\textrm{push}}$ penalizes $\mathbf{L}$ unless the distance between $\mathbf{L}\mathbf{\widetilde{S}}\boldsymbol{x}$ and $\mathbf{L}\mathbf{\widetilde{S}}\boldsymbol{y}$ is smaller than the distance between $\mathbf{L}\mathbf{\widetilde{S}}\boldsymbol{x}$ and $\mathbf{L}\mathbf{\widetilde{S}}\boldsymbol{z}$ by a margin of at least $1$.
...
In doing so, it is in line with a recent publication \cite{wang2020icassp}, in which the authors successfully trained a supervised classifier on joint time--frequency scattering features in order to detect and classify playing techniques from the Chinese bambooo flute (\emph{dizi}). \ml{limiter les self citations a une occurence}
...
Indeed, \ml{we have strong assumptions} that the tools used (scattering transforms and LMNN weighting matrices) do not depend on the type of sound being processed. \ml{Though, future work should demonstrate that they} work for more general classes of sounds.
...
Same 3 sounds spectrogram + scattering
% TODO figure + caption
}
\label{fig:sample-scattering}
\end{figure}
