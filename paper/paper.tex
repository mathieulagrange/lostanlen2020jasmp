...
The \lnameref{sec:survey} section describes our computer--human interface for annotating auditory similarities across playing techniques.
...
\subsection*{Per-feature Gaussianization with median-based logarithmic compression}
...
Given a collection of $N$ such waveforms, we number them arbitrarily from $\boldsymbol{x}_1$ to $\boldsymbol{x}_N$. Then, for every path $p$ in the joint time--frequency scattering transform operator $\mathbf{S}$, we measure the median value taken by the average response of $\mathbf{S}\boldsymbol{x}_n$ across all samples $n$ from $1$ to $N$:
\begin{equation}
\boldsymbol{\mu}(p) =
...
Lastly, we standardize every feature $\mathbf{\widetilde{S}}\boldsymbol{x}_n$ to null mean and unit variance, across all $N$ samples $\boldsymbol{x}_1 \ldots \boldsymbol{x}_N$, independently for each scattering path $p$.
Again, in cases where the dataset is split between training and test sets, we measure means and variances over the test set and propagate them as constants to the test set.
