...
On the other hand, the sole mention of a playing technique does not determine any intended effect; as a result, it fails to generalize to the entire instrumentarium \cite{kolozali2011ismir}.
...
In this context, the computer serves as a bridge from the auditory level to the symbolic level, \ie{} from a potentially infinite realm of timbral sensations to a musical score of predetermined range.
...
These visualizations confirm the theoretical predictions of previous research \cite{anden2019tsp}, which relied on simple synthetic sounds such as exponential chirps.
...
\ja{These section numbers aren't showing up since we're not numbering the sections (per the style guide).}
...
For instance, the Open-MIC dataset \cite{humphrey2018ismir}, maintained by the Community for Open and Sustainable Music Information Research (COSMIR) \cite{mcfee2016ismir}, comprises a vast corpus of 20000 polyphonic music recordings spanning 20 instruments as a derivative of the Free Music Archive (FMA) dataset \cite{defferrard2017ismir}.
...
\ja{This is quite an abrupt transition.
Why ``nevertheless''?}%
...
These biologically inspired features  outperform the state of the art, especially in the small data regime when deep learning is inapplicable.
...
Recently, Wang et al.~\cite{wang2019ismir} trained a classifier of playing techniques on separable (\ie{} non-joint) time--frequency scattering features \cite{anden2014deep}, and achieved state-of-the-art results in continuous performance recordings of the Chinese bamboo flute.
...
\ja{I don't understand this last sentence.
...
\ja{This section is way too long.
It's important that we situate ourselves with respect to the existing literature, but we don't need to spend 2+ pages doing so.
This currently reads more like a laundry list of various work related to timbre perception on musical instruments on one hand and a history of STRFs on the other.
We need to keep in mind what our central themes (wider range of timbre, similarity-based evaluation, simple scattering+linear model) are and link the existing work to them in a concise manner.
}
...
The second component warps this timbre space by taking into account perceptual judgments represented as a consensus clustering of the items in the dataset, as presented in Section \ref{sec:survey}.
This warping, or reweighting, of the space is performed using LMNN.
\ja{Again, section numbers are not showing up here.}%
...
% cite Lostanlen PhD dissertation. classification results on Medley-solos-DB. suggests that JTFS is competitive wrt deep learning techniques even in the large data regime (500-5000 samples per class).
...
We refer to \cite{x} and \cite{x} for a review of recent literature on the timbre modeling of percussive sounds, from the standpoints of MIR and music cognition respectively. % TODO VL CITE
\ja{Is it really necessary to go into such detail only to explain why we're not dealing with percussive sounds?
One sentence should be enough.}%

We consider a list of 16 instruments: violin (Vn), viola (Va), cello (Vc), contrabass (Cb), concert harp (Hp), Spanish guitar (Gtr), accordion (Acc), flute (Fl), soprano clarinet (BbCl), alto saxophone (ASax), oboe (Ob), bassoon (Bn), C trumpet (TpC), French horn (Hn), tenor trombone (TTbn), and bass tuba (BBTb).
Among this list, the first six are string instruments, the next five are woodwinds, and the last five are brass instruments.
...
Once augmented with mutes, the list of 16 instruments grows up to X. % TODO complete
Furthermore, every instrument, whether equipped with a mute or not, affords a panel of playing techniques ranging in size between X (for the X) and Y (for the Y). % TODO update with number.
In total, the feasible triplets of instrument, mute, and technique amount to a list of $N=235$ combinations.

% TODO explain the conundrum leading to reduce the dataset size.
...
Each dot corresponds to a different IPT recording and is moved by the user dragging them with the mouse.
In addition to spatially arranging the dots, the user is also asked to assign one of $20$ colors to each dot, providing a simple measure of clustering of the sounds.
...
\caption{Spatial and group organization provided by subject 1. Other subjects are available at \url{https://mathieulagrange.github.io/paperPlayingTechniquesSimilarityLearning/demo}. For each subject, the stimuli can be played back by hovering on the dots.}
...
In other words, the clustering assignment of each subject as a binary similarity matrix.
...
In this study, we choose to condition the space using the aggregate similarity judgments given by several subjects.
...
In order to better interpret the information captured by the consensus clustering, we generate two similarity matrices, one for the 15 instruments used (see Figure \ref{fig:consensusVsI}) and one for the 23 PTs used (see Figure \ref{fig:consensusVsPt}).
For the instrument similarity matrix, the value within each cell counts the number of times two instruments have been grouped in the consensus clustering, normalized by the average number of IPTs that involved the two instruments.
The PT similarity matrix is computed in the same way.

The ordering of the instruments or the PTs on the vertical and horizontals axes is chosen according to the ordering given by an agglomerative hierarchical clustering using the ward criterion of the similarity matrix under inspection.
This ordering allows us to interpret the similarity matrices more easily compared to an alphabetical ordering while preserving the  similarity values between each pair of instruments or PTs.
\ja{Do we need to talk about this?
It's just for visualization.
If we want to keep it, we should provide references to the methods used.}%
...
\ja{Should we loop back here and explain how the consensus clustering is used as input to the LMNN?
Feels like we've lost the thread a bit here otherwise.}%
...
The processing pipeline presented in Figure \ref{fig:pipeline} addresses the problem of conditioning a timbre space using perceptual judgments of timbral similarity.
The relevance of the many design choices that had to be made are addressed in this section using an experimental protocol that considers a dataset of audio that will be given as input to the processing pipeline and one metric used for evaluating its performance under a given experimental setting.
\ja{This last sentence is quite vague.
...
For each sound $s_q, 0<q<9346$ considered as a query, its 5 nearest neighbors $s_n, 0<n<5$ in the timbre space under evaluation are retrieved using the Euclidean distance between corresponding features vectors.
...
The best performing system considers the separable scattering computed over $1000~\mathrm{ms}$ time support, projected using the LMNN algorithm.
\ja{Separable scattering?
Not the joint?}%
Evaluated over the entire dataset, that is the computation of the projection and the evaluation are performed on the whole dataset, this system reaches the satifactory performance of $99.5\%$ of $ap@5$.
...
By considering this protocol, the proposed system reaches a $ap@5$ of $95.5\%$.
If considering the same first half both for learning and evaluation leads to a $ap@5$ of $96.5\%$, indicating a low risk of overfitting.
...
In this case, the performance drops to $23.6\%$.
...
When considering LDA in place of LMNN, the performance drops to $81.2\%$.
...
When considering the commonly used duration of $25~\mathrm{ms}$, the performance drops down to $92\% \pm$.
\ja{$\pm$ what?}%
This demonstrates that the joint scattering is able to benefit from a larger time support, possibly to capture longer-term modulations which play an important part when considering extended playing techniques \cite{lostanlen2018extended}.
\ja{Other references?}%
...
Further simplifying the feature set by replacing the scattering coefficients with MFCCs gives a performance of $82.3\% \pm 7.1$.
...
To do so, the $40$ features $f_i$ of the MFCCs are multiplied together $f_{i, j} = f_i*f_j, i \neq j$.
\ja{What are the $f_i$ here?
How are they defined?}%
The resulting features set consists of the 40 MFCCs features $f_i$ and the $1140$ features $f_{i, j}$.
The resulting $ap@5$ of $81.8\% \pm 7$ indicates that the increase of dimensionality is has a small effect, confirming the relevance of the additional information provided by the joint scattering.
