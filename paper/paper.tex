...
As gathering perceptual similarity data is plagued with a size problem. the perceptual data gathered during the perceptual experiment considers 78 \ipts each represented by one sound recording. In order to provide enough data for the numerical analysis described in this section, we assume that the impact of the pitch and nuance on perceptual similarity is lower than the timbral similarity constraints expressed by the subjects. Assuming invariance to timbre with respect to pitch and nuance is in line with the ansi definition of timbre, it is  thus taken in this paper as a reasonable assumption. Quantification of this invariance is left for further study.

This result in considering for each of the 78 \ipts the whole range of nuance and pitch, leading to a dataset composed of 9346 sound recordings where two sounds of this dataset $a$ and $b$, corresponding to \ipts $I_a$ and $I_b$ are considered as similar with respect to a given reference clustering $c_i$ if the \ipts $I_a$ and $I_b$ are grouped in the same cluster.

The metric is the average precision at rank $k=5$ (ap@5), that allows us to evaluate if the outputs of the proposed system are likely to satisfy the user.

The reference considered here is thus the $31$ clusterings obtained with the perceptual experiment, see Section \ref{sec:judgments}. for each of those clusterings, the precision at rank $5$ is computed, and the $31$ p@5 are averaged to obtain the ap@5.
...
The processing pipeline is composed of 2 main building blocks. The first takes a digital audio recording as input and outputs one vector of features.

The second considers one vector of features as input and another vector of project features as output.
