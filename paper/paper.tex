...
The name Cyberlioz is a portmanteau between the prefix cyber- and the French composer Hector Berlioz, whose \emph{Treatise on Orchestration} (1844) marked a watershed in the history of European classical music, because of its deliberate focus on the role of timbre as a parameter for musical expression.
Cyberlioz consists of a square panel on which is displayed a collection of circular dots, each of them corresponding to one of the $N=78$ IMTs.
Hovering the screen pointer onto each dot results in a playback of the representative audio sample of this IMT, \ie{} C4 and \emph{mf} in most cases. % TODO VL: use pitch notation library to express C4
