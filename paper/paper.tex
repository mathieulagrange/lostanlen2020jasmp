...
for pairs of samples belonging to the same cluster, while the term $\mathcal{E}_{\textrm{push}}$ tends to ensure that the distance between 
...
\sum_{\boldsymbol{x}\in\mathcal{X}}
\sum_{\boldsymbol{y}\in\mathcal{Y}_R (\boldsymbol{x})}
\sum_{\substack{\boldsymbol{z}\in\mathcal{Y}_R (\boldsymbol{x}) \\ \boldsymbol{z}\not\in\mathcal{G}(\boldsymbol{x})}}
\rho
\Big(
1 +
\big\Vert
...
\Big),
\end{equation}
where the function $\rho : u \mapsto \max (0, u)$ denotes the activation function of the rectified linear unit (ReLU).
