...
The notion of musical timbre have been extensively considered through the lens of the classic instrumentarium. For such a purpose, datasets of instruments played \textit{ordinario} are considered. On thoses datasets, computational model of timbre considering static spectral features such as Mel Frequency Cepstral coefficents displayed excellent performance.

The availability of richer musical datasets with many playing techniques, allows us to consider computational modeling of musical timbre with the outcomes of recent study  that stresses the importance of long term modulations in auditory perception.

We propose a timbre model that is based on a parametrized...

This model is evaluated in a retrieval task using a perceptually defined retrieval objective. Computational experiments demonstrates 1) the importance of handling long term modulations both in time and frequency, with a gain of $14\%$ in precision compared to Mfccs  2) the relevance of a learned layer that is able provide an average gain of $3\%$ in precision with a reduction of the variance of the prediction error of a factor of 2.
...
It is composed of two stages, one fixed and one learned, the former providing a baseline model for audio perception, with the latter specifically adapting the representation for estimation of timbre similarity.
The fixed stage is constructed using the joint time--frequency scattering transform introduced by And\'en et al. \cite{anden2015joint,anden2019joint}.
This representation extracts the time--frequency structure of the audio signal in a manner invariant to small time-shifts and stable to time-warping deformations.
As such, it provides a general description of the audio signal that is not necessarily tailored to measuring timbre similarity.
The second stage consists of a learned linear transform that, applied to the scattering representation, induces a metric that more closely aligns with timbre similarity judgments extracted from human listeners.
We see that, numerically, this learned metric almost exactly reproduces the human similarity judgments.
Moreover, we perform an ablation study of the model showing that degrading any element of it (replacing the scattering transform by simpler spectral descriptors, for example) significantly reduces the accuracy of the fit.
