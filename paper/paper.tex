...
In practice, for training and evaluation purposes, the cluster graph $\mathcal{G}$ may stem from the consensus of $K=31$ users of the Cyberlioz web application, which was described in the \lnameref{sec:survey} section ($\mathcal{G}=\mathcal{G}_0$).
Alternatively, it may correspond to the subjective preferences of a single user $k$, in which case we take $\mathcal{G}=\mathcal{G}_k$.
...
Our topline system comprises four computational blocks:
\begin{enumerate}
...
\item Median-based logarithmic compression \cite[Equation 1]{lostanlen2018extended}, and
