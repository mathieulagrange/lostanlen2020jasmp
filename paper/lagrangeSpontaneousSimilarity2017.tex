...
We therefore now only study the properties of the groupings performed by the subject prior converting them into an overall similarity matrix.
...
A partition of $n$ clusters can be constructed from the agglomerative hierarchical cluster tree by finding the smallest height at which a horizontal cut through the tree leaves $n$ clusters. In order to
...
\caption{Clustering \ipt using average perceptual similarity into 5 clusters (a) and playing technique labels (b). The colors of clusters displayed on top corresponds to the playing technique that is dominant in this cluster.}
\label{fig:xp2sizeGroup}
\end{figure}
...
\caption{Clustering \ipt using average perceptual similarity into 8 clusters (a) and instrument reference labels (b). The colors of clusters displayed on top corresponds to the intrument that is dominant in this cluster.}
\label{fig:xp2sizeGroup}
\end{figure}
