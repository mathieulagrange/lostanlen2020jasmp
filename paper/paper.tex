...
In practice, for training and evaluation purposes, the graph $\mathcal{G}_0$ stems from the consensus of $K=31$ users of the Cyberlioz web application, which was described in the \ref{sec:survey} section.
...
In music information retrieval, the core motivation behind timbre similarity modeling lies in the prospect of navigating an online corpus of digital music.
The typical size of such a corpus ranges between $N\sim10^3$ and $N\sim10^7$, depending on the use case.
As of today, the two largest databases of isolated musical notes with extended playing techniques are FullSOL and conTimbre, with $N\sim1.2\cdot10^5$ and $N\sim8.7\cdot10^4$ samples respectively.% TODO cite FullSOL and conTimbre
In comparison, the instrument--mute--technique (IMT) triplets in the present study on human subjects amount to merely $N=78$ samples, \ie{} three orders of magnitude less than conTimbre.
%even though there is expert pre-screening, etc.
% does not reflect the typical scale at which these systems are deployed.
